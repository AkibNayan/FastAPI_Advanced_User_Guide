...
async def stream_story() -> AsyncIterable[str]:
    for line in message.splitelines():
        yield line

@app.get("/story/stream_no_async", response_class=StreamingResponse)
...
def stream_story_no_annotation_no_async():
    for line in message.splitlines():
        yield line
...
def stream_story_no_annotation_no_async_bytes():
    for line in message.splitlines():
        yield line.encode("utf-8")